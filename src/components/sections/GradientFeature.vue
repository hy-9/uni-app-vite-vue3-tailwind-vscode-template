...
const gradientShowcase = [
	{
		...
		classes:
			"bg-[radial-gradient(circle_at_top,#22d3ee,#0f172a)] text-white",
	},
	{
		name: "玻璃注脚",
		desc: "bg-white/70 backdrop-blur-xl border border-white/40",
		classes:
			"bg-white/70 border border-white/40 text-slate-700 backdrop-blur-xl",
	},
	{
		name: "霓虹波纹",
		desc: "bg-[conic-gradient(at_top,#0ea5e9,#6366f1,#a855f7)]",
		classes:
			"bg-[conic-gradient(at_top,#0ea5e9,#6366f1,#a855f7)] text-white",
	},
];
...
<template>
	...
	>
		...
		<view class="mt-5 text-sm text-slate-500"> feature cards </view>
		<view class="mt-3 flex flex-col gap-4 md:grid md:grid-cols-2">
			<view
				v-for="feature in featureCards"
				:key="feature.title"
				class="rounded-[28rpx] border border-slate-100/80 bg-gradient-to-br p-4 shadow-lg"
				:class="feature.backdrop"
			>
				<view class="flex items-center gap-3">
					<text
						class="text-[36px]"
						:class="[feature.icon, feature.accent]"
					/>
				...
				<view
					class="mt-3 inline-flex rounded-full bg-white/70 px-3 py-1 text-xs font-semibold text-slate-600"
				>
					{{ feature.chip }}
				</view>
			</view>
