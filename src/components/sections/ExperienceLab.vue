...
<template>
	<view
		class="rounded-[32rpx] border border-slate-100/70 bg-white/90 p-5 shadow-[0_20px_45px_rgba(15,23,42,0.08)]"
	>
		...
		<view class="mt-4 flex flex-col gap-4 lg:grid lg:grid-cols-3">
			<view
				class="flex flex-col gap-3 rounded-[28rpx] border border-slate-200/80 bg-gradient-to-br from-slate-900/95 to-slate-700/95 p-4 text-white shadow-xl"
			>
				...
				<button
					class="w-full rounded-2xl py-2 text-base font-semibold shadow-lg transition hover:scale-[1.01]"
					:class="buttonClass"
					@click="increment"
				>
					click to inc
				...
				<view class="test"> @apply + 自定义尺寸 </view>
			</view>

			<view
				class="flex flex-col gap-3 rounded-[28rpx] border border-slate-200/80 bg-white/85 p-4 shadow-xl backdrop-blur-lg"
			>
				...
				<view
					class="mt-4 rounded-2xl border border-dashed border-slate-200 bg-slate-50/70 p-3 text-[26rpx] text-slate-600"
				>
					themeRef: {{ themeRef }}
				</view>
				<view class="mt-4">
					...
					<view class="mt-2 flex flex-wrap gap-2 text-xs">
						<view
							class="rounded-full bg-slate-900/5 px-3 py-1 font-semibold text-slate-600"
							@click="
								copy(
			...
			>
				...
				<view
					class="mt-3 flex h-16 w-32 items-center justify-center rounded-[20rpx] bg-[#389f2bb1] text-white after:content-['hover_here!']"
				...
				<view
					class="group relative rounded-2xl bg-green-200/70 p-[60px] text-xs text-slate-700 before:absolute before:left-1 before:top-1 before:text-slate-500 before:content-['父元素']"
					hover-class="published"
				>
					<view
						class="rounded-xl bg-pink-400/80 p-2 text-white ring-4 ring-pink-200 group-[.published]:bg-yellow-400 group-[.published]:text-slate-800"
					>
		...
		<view class="mt-5 flex flex-col gap-4 md:grid md:grid-cols-2">
			<view
				class="rounded-[26rpx] border border-slate-200/80 bg-white/70 p-4 shadow-inner"
			>
				<view class="text-sm font-semibold text-slate-700">
					Tailwind ring + 自定义按钮
				</view>
				<view class="text-xs text-slate-500"> ring utilities </view>
				<view class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
					<view
						class="w-full rounded-md bg-pink-500 py-2 text-center font-semibold text-white ring-4 ring-pink-300"
					>
						Default Ring
					</view>
					<button class="btn">@layer 定制按钮</button>
				</view>
			</view>
			...
			>
				...
				<view class="mt-3 space-y-2">
					<view
						...
						:key="tip.title"
						class="rounded-2xl border border-slate-100/80 bg-slate-50/60 px-3 py-2 text-[26rpx] text-slate-600"
					>
				...
				<view
					class="mt-4 text-neutral-600 underline"
					@click="copy('https://tw.icebreaker.top/docs/icons')"
				>
